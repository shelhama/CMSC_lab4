   ...
   public void printArray() {
	    for (int row = 0; row < intArray.length; row++) {
	        for (int col = 0; col < intArray[row].length; col++) {
	            ...
	            animate(row, col); // animate after printing each element
	        }
   ...
   public void setValues( int value, int row )
   {
    ...
    // Write your code here:
		    ...
		    for (int col = 0; col < intArray[row].length; col++) {
		        ...
		        animate(row, col); // animate after each change
		    }
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
		    ...
		    // Assume the first element in the column is the minimum
		    ...
		    for (int row = 1; row < intArray.length; row++) {
		        ...
		        if (intArray[row][column] < min) {
		            min = intArray[row][column];  // Update the minimum value
		        }

		        // Animate the current row and column being checked
		        animate(row, column, min);  // Animate with the current minimum value
		    }
		    ...
		    for (int row = 0; row < intArray.length; row++) {
		        for (int column = 0; column < intArray[row].length; column++) {
		            if (intArray[row][column] == value) {
		                count++;  // Increment the count when the value is found
		            }
		            // Animate the current row, column, and the count (current frequency)
		            animate(row, column, count);  // Pass the current count to animate
		        }
		    }
